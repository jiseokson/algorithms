...
int main(void)
{
	...
	newPerson = makePersonData(20130012, "Son", "SungNam");
	insertTable(&table, getSSN(newPerson), newPerson);
	
	newPerson = makePersonData(20170012, "Park", "AnSung");
	insertTable(&table, getSSN(newPerson), newPerson);
	...
	targetPerson = searchTable(&table, 20130012);
	if (targetPerson != NULL)
		showPersonInfo(targetPerson);
	putchar('\n');
	
	targetPerson = searchTable(&table, 20170012);
	if (targetPerson != NULL)
		showPersonInfo(targetPerson);
	putchar('\n');
	...
	delPerson = deleteTable(&table, 20130012);
	if (delPerson != NULL)
		free(delPerson);
		
	delPerson = deleteTable(&table, 20170012);
	if (delPerson != NULL)
		free(delPerson);
