...
위를 변형해 index k번째 답만 출력하는 알고리즘을 작성하자. 이때 빈문자열을 index 0이라 가정했다.
```C++
...
위의 구현은 문자열이 완성된 경우에 k가 하나씩만 넘어간다. 효율성을 높이기 위해 문자열을 생성하기 전에 앞으로 생성하게 될 문자열 중 원하는 문자열이 없으면 넘어가도록 구현을 바꿔보자.
```C++
int search(string res, int len) // len: 더 첨가할 수 있는 문자의 개수
{
    if (k == 0) {
